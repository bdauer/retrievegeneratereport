...
class BuilderTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = [{"site_name":"apple",
                ...
                "word_count": 145}]
        categories = []
        self.r = ReportBuilder("Report", categories, data)
        ...
        self.wcr_table = """<table border="1">
                    ...
                    <tr><td><b>avg word count:</b></td><td><b>93.5</b></td></tr>
                    </table>""".replace("\n", "").replace(" ", "")

    def test_plain_table(self):
        plain_table = self.r.build_html_table().replace("\n", "").replace(" ", "")
        self.assertEqual(plain_table, self.plain_table)

    def test_plain_report(self):
        plain_report = self.r.build_report("html").replace("\n", "").replace(" ", "")
        self.assertEqual(plain_report, self.plain_report)


    def test_word_count_table(self):
        wcr_table = self.wcr.build_html_table().replace("\n", "").replace(" ", "")
